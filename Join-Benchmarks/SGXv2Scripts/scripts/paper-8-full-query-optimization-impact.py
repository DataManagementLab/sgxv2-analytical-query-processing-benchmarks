...
def paper_plot_stacked(filename_detail, experiment_name):
    ...
    data["Setting"] = (data["mode"] + data["Unrolled"].astype(str)).replace({
        ...
        "tpchFalse": "SGX Data in Enclave",
        "tpchTrue": "SGX Data in Enclave opt."
    })
    setting_order = {"Plain CPU": 0, "Plain CPU opt.": 100, "SGX Data in Enclave": 200,
                     "SGX Data in Enclave opt.": 300}

    means = data.groupby(["mode", "Unrolled", "Query", "Phase"])["Runtime in ms"].mean()

    default_sgx = means["tpch", False]
    default_sgx_totals = default_sgx.groupby(level=0).sum()
    optimized_sgx = means["tpch", True]
    optimized_sgx_totals = optimized_sgx.groupby(level=0).sum()
    plain = means["tpch-native", False]
    plain_totals = plain.groupby(level=0).sum()

    sgx_improvement = 1 - (optimized_sgx_totals / default_sgx_totals)
    sgx_overhead = default_sgx_totals / plain_totals
    sgx_optimized_overhead = optimized_sgx_totals / plain_totals
    ...
    f, ax = plt.subplots(figsize=(6, 3.75))
    (so.Plot(data, x="Query", y="Runtime in ms", color="Setting", alpha="Phase")
     .add(so.Bar(), so.Agg(), so.Dodge(by=["color"]), so.Stack())
     .scale(
        color=[sns.color_palette("deep")[0], sns.color_palette("deep")[2], sns.color_palette("deep")[3]],
    ...
    move_legend_fig_to_ax(f, ax, loc="upper right")

    plt.grid(axis="y")
    plt.tight_layout()
    plot_filename = f"../img/paper-figure-full-query-optimization.pdf"
