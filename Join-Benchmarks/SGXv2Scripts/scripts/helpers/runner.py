...
class ExperimentConfig:
    ...
    def run_count(self):
        return (len(self.modes) * len(self.flags) * len(self.sizes) * len(self.algorithms) * len(self.threads)
                * len(self.materialize) * self.repetitions * len(self.init_core))
...
class JoinRunConfig:
    ...
    @classmethod
    def header(cls) -> str:
        return "mode,flags,alg,materialize,threads,size_r,size_s,init_core,dynamic_enclave"
            ...
            try:
                ...
                result = subprocess.run(
                    [f"./{exe[1]}", "-a", config.alg, "-r", str(config.size_r), "-s", str(config.size_s),
                     "-n", str(config.threads), "-c", str(config.init_core)] + materialize_flag,
                    cwd=f"../../{exe[0]}/",
                    env=os.environ | {"SGX_DBG_OPTIN": "1"},
                    capture_output=True,
                    text=True,
                            ...
                            for materialize in config.materialize:
                                for init_core in config.init_core:
                                    ...
                                    run_config = JoinRunConfig(mode, flag_set, algorithm, size[0], size[1], threads,
                                                               materialize, init_core, dyn_enclave)
