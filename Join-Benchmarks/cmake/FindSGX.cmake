#source: https://github.com/xzhangxa/SGX-CMake/blob/master/cmake/FindSGX.cmake
# FindPackage cmake file for Intel SGX SDK

cmake_minimum_required(VERSION 3.16) # target_include_directories
...
if(CMAKE_SIZEOF_VOID_P EQUAL 4)
    set(SGX_COMMON_CFLAGS "-m32 -march=native")
    ...
    set(SGX_COMMON_CFLAGS "-m64 -march=native")
...
if(SGX_FOUND)
    ...
    if(SGX_MODE STREQUAL "Debug")
        message(STATUS "SGX Debug Mode")
    elseif(SGX_MODE STREQUAL "PreRelease")
        message(STATUS "SGX PreRelease Mode")
    elseif(SGX_MODE STREQUAL "Release")
        message(STATUS "SGX Release Mode")
    else()
    ...
    set(ENCLAVE_C_FLAGS "${SGX_COMMON_CFLAGS} -nostdinc -fvisibility=hidden -fpie -fstack-protector-strong -DENCLAVE")
    ...
    function(_build_edl_obj edl)
        set(optionArgs "")
        set(oneValueArgs USE_PREFIX HEADER_ONLY)
        set(multiValueArgs SEARCH_PATHS DEPENDENCIES)
        cmake_parse_arguments("EDL" "${optionArgs}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
        ...
        if(${EDL_HEADER_ONLY})
            set(HEADER_ONLY_STRING "--header-only")
        endif()
        add_custom_command(OUTPUT ${EDL_T_C}
                COMMAND ${SGX_EDGER8R} ${USE_PREFIX} ${HEADER_ONLY_STRING} --trusted ${EDL_ABSPATH} --search-path ${SEARCH_PATHS}
        ...
        if(EDL_DEPENDENCIES)
            message("Adding dependencies to ${edl}")
            target_link_libraries(${target}-edlobj ${EDL_DEPENDENCIES})
        endif()
    ...
    function(add_trusted_library target)
        set(optionArgs USE_PREFIX USE_SGXSSL HEADER_ONLY)
        set(oneValueArgs EDL LDSCRIPT)
        set(multiValueArgs SRCS EDL_SEARCH_PATHS EDL_DEPENDENCIES)
        ...
        if("${SGX_EDL}" STREQUAL "")
            ...
            _build_edl_obj(${SGX_EDL} SEARCH_PATHS "${SGX_EDL_SEARCH_PATHS}" USE_PREFIX ${SGX_USE_PREFIX} HEADER_ONLY ${SGX_HEADER_ONLY} DEPENDENCIES "${SGX_EDL_DEPENDENCIES}")
        ...
        target_link_libraries(${target} "${SGX_COMMON_CFLAGS} \
            ...
            -Wl,--start-group -lsgx_tstdc -lsgx_tcxx -lsgx_tkey_exchange -lsgx_tcrypto -lsgx_pthread -l${SGX_TSVC_LIB} -Wl,--end-group \
    ...
    function(add_enclave_library target)
        ...
        set(oneValueArgs EDL LDSCRIPT)
        set(multiValueArgs SRCS TRUSTED_LIBS EDL_SEARCH_PATHS EDL_DEPENDENCIES)
        cmake_parse_arguments("SGX" "${optionArgs}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
        if("${SGX_EDL}" STREQUAL "")
        ...
        _build_edl_obj(${SGX_EDL} SEARCH_PATHS "${SGX_EDL_SEARCH_PATHS}" USE_PREFIX ${SGX_USE_PREFIX} HEADER_ONLY 0 DEPENDENCIES "${SGX_EDL_DEPENDENCIES}")
        ...
        target_link_libraries(${target} "${SGX_COMMON_CFLAGS} \
            ...
            -Wl,--start-group ${TLIB_LIST} -lsgx_tstdc -lsgx_tcxx -lsgx_tkey_exchange -lsgx_pthread -lsgx_tcrypto -l${SGX_TSVC_LIB} -Wl,--end-group \
            -Wl,-Bstatic -Wl,-Bsymbolic -Wl,--no-undefined \
            -Wl,-pie,-eenclave_entry -Wl,--export-dynamic \
            ${LDSCRIPT_FLAG} \
            -Wl,--defsym,__ImageBase=0")
    ...
    function(add_untrusted_library target mode)
        set(optionArgs USE_PREFIX HEADER_ONLY)
