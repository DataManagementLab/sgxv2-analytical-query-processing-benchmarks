 ...
 */
void
build_hashtable_mt_no_overflow_unrolled(hashtable_t *ht, const table_t *rel) {
    const uint32_t hash_mask = ht->hash_mask;
...
template<HashTableBuildFunction BuildFunction, HashTableProbeFunction ProbeFunction>
result_t *
PHT_no_template(const table_t *relR, const table_t *relS, const joinconfig_t *config) {
    hashtable_t *ht;
    int64_t result = 0;
    int32_t numR;
    int32_t numS;
...
#ifdef UNROLL
    return PHT_no_template<build_hashtable_mt_no_overflow_unrolled, probe_hashtable_no_overflow_unrolled>(relR, relS,
                                                                                                          config);
#else
...
PHT_no(const table_t *relR, const table_t *relS, const joinconfig_t *config) {
    return PHT_no_template<build_hashtable_mt_no_overflow, probe_hashtable_no_overflow>(relR, relS, config);
}
