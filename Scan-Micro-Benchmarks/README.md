...
The result figures are stored in `img`. The other results from `PMBW`, `RadixPartitioning`, and `WriteBench` can be 
reproduced with the same process. Exceptions: 

- For PMBW, result files are `.txt` files that can be converted using `pmbw_convert_csv.py` after they have been copied
  to the `results/data` directory.
- To create the random access figure, run `PMBW/1_pmbw_random.sh` and `WriteBench/0_multi_write.sh` and put all result 
  files into `PMBW/results/data`. Then use `PMBW/results/plot.py` to plot.
