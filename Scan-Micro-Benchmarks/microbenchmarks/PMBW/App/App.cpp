...
#include "Enclave_u.h"
#include "../Enclave/structs.h"

constexpr char ENCLAVE_FILENAME[] = "pmbwenclave.signed.so";
...
int initialize_enclave(const char *enclave_file) {
    ...
    ret = sgx_create_enclave(enclave_file, 1, nullptr, nullptr, &global_eid, nullptr);
    if (ret != SGX_SUCCESS) {
...
static void cpuid_detect() {
    ...
    if (cpuid_avx512()) ERRX(" avx512");
    ERR("");
}

void ocall_cpuid_detect(int *cpuid_op1){
    cpuid(1,cpuid_op1);
}
...
void print_usage(const char *prog) {
    ERR("Usage: " << prog << " [options]" << std::endl
                  << "Options:" << std::endl
                  << "  -e             Run Benchmark inside Enclave" << std::endl
                  << "  -f <match>     Run only benchmarks containing this substring, can be used multiple times. Try \"list\"."
                  << std::endl
...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt(argc, argv, "hf:eM:o:p:P:Qs:S:")) != -1) {
        switch (opt) {
            default:
            case 'h':
                print_usage(argv[0]);
    ...
    ERR("Detected " << physical_mem / 1024 / 1024 << " MiB physical RAM and " << g_physical_cpus << " CPUs. "
                    << std::endl);
    ...
    if (gopt_enclave) {
        gopt_parameters params;
        params.g_physical_cpus = g_physical_cpus;
        params.gopt_testcycle = gopt_testcycle;
        params.gopt_nthreads_exponential = gopt_nthreads_exponential;
        params.gopt_nthreads_max = gopt_nthreads_max;
        params.gopt_nthreads_min = gopt_nthreads_min;
        params.gopt_sizelimit_min = gopt_sizelimit_min;
        params.gopt_sizelimit_max = gopt_sizelimit_max;
        params.gopt_funcfilter = const_cast<char **>(gopt_funcfilter.data());
        params.gopt_funcfilter_size = gopt_funcfilter.size();
        params.g_memsize = g_memsize;
        auto init_ret = initialize_enclave(ENCLAVE_FILENAME);
        if (init_ret != 0) {
            return -1;
        }
        unlink(gopt_output_file);
        auto ret = ecall_main(global_eid, params);
        int return_code = 0;
        if (ret != SGX_SUCCESS) {
            std::cerr << "Running ecall_main failed!" << std::endl;
            print_error_message(ret);
            return_code = -1;
        }
        destroy_enclave();
...
void ocall_print_string(const char *X) {
    ERR(X);
}

void ocall_print_result(const char *name, result_struct resultStruct) {
    std::ostringstream result;
    result << "RESULT\t";
    ...
    result << "datetime=" << datetime << '\t'
           ...
           //<< "version=" << PACKAGE_VERSION << '\t'
           << "funcname=" << name << '\t'
           << "nthreads=" << resultStruct.nthreads << '\t'
           << "areasize=" << resultStruct.areasize << '\t'
           << "threadsize=" << resultStruct.threadsize << '\t'
           << "testsize=" << resultStruct.testsize << '\t'
           << "repeats=" << resultStruct.repeats << '\t'
           << "testvol=" << resultStruct.testvol << '\t'
           << "testaccess=" << resultStruct.testaccess << '\t'
           << "time=" << std::setprecision(20) << resultStruct.runtime << '\t'
           << "bandwidth=" << resultStruct.bandwidth << '\t'
           << "rate=" << resultStruct.rate;
