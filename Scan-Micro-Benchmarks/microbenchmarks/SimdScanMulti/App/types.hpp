...
[[nodiscard]] Trinary from_string(const std::string &value);
...
public:
    ...
    ParameterSpan(T min, T max, T step, bool exp_steps) : min{min}, max{max}, step{step}, exp_steps{exp_steps},
    current{min} {
        if (max < min) {
...
struct Configuration {
    ...
    Configuration() : read_config{true, 0, 1}, predicate_high{static_cast<uint8_t>(std::round((static_cast<double>(selectivity) / 100.0) * 255.0))},
                      num_entries_per_thread{num_entries / num_threads} {}

    Configuration(bool enclave,
                  bool preload,
                  Mode mode,
                  MultiReadConfiguration read_config,
                  bool numa,
                  ...
                  prefer_SMT(prefer_smt), num_threads(num_threads), num_entries(num_entries), num_reruns(num_reruns),
                  selectivity(selectivity), include_join(include_join), pre_alloc(pre_alloc),
                  predicate_high{static_cast<uint8_t>(std::round(
                          (static_cast<double>(selectivity) / 100.0) * 255.0)
                          )},
                  num_entries_per_thread{num_entries / num_threads}
                  {}
};
                ...
                for (auto mode: modes) {
                    for (auto unique_data : convert_trinary(unique_data)) {
                        for (auto numa : convert_trinary(NUMA)) {
                            for (auto smt : convert_trinary(prefer_SMT)) {
                                    ...
                                    for (auto ent : num_entries.to_vector()) {
                                        for (auto sel : selectivity. to_vector()) {
                                            Configuration config {
                                                ...
                                                {
                                                    unique_data,
                                                    static_cast<uint8_t>(!unique_data ? num_warmup_runs : 0),
                                                    !unique_data ? num_runs : 1
                                                },
                                                numa,
                                                smt,
                                                thrds,
