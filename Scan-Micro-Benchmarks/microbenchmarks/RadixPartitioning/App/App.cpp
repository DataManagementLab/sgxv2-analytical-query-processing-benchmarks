...
DEFINE_bool(unroll, false, "Unroll loops on/off");
...
int main(int argc, char *argv[]) {
    ...
    ret = ecall_init_data(eid, data_size, max_radix_bits, max_padding);
    if (ret != SGX_SUCCESS) {
    ...
    for (uint32_t radix_bits = min_radix_bits; radix_bits <= max_radix_bits; ++radix_bits) {
        ...
        for (uint32_t num_keys_exp = FLAGS_min_num_keys_exp; num_keys_exp <= FLAGS_max_num_keys_exp; ++num_keys_exp) {
            for (uint32_t repeat = 0; repeat < FLAGS_repeat; ++repeat) {
                ...
                params.setParam("unroll", FLAGS_unroll ? "yes" : "no");

                uint64_t hist_cycle_counter = 0;
                if (FLAGS_unroll) {
                    PerfEventBlock e{data_size, params, print_header, &hist_cycle_counter};
                    histogram_unrolled(data, data_size, mask, shift, hist,
                                       &hist_cycle_counter);
                } else {
                    PerfEventBlock e{data_size, params, print_header, &hist_cycle_counter};
                    histogram(data, data_size, mask, shift, hist,
                              &hist_cycle_counter);
                }
                print_header = false;
                ...
                if (FLAGS_user_check) {
                    ...
                    if (FLAGS_unroll) {
                        ...
                        PerfEventBlock e{data_size, params, print_header, &hist_cycle_counter_user_check};
                        ecall_histogram_user_check(eid, data, data_size, hist, mask, shift,
                                                   &hist_cycle_counter_user_check);
                    }
                }
                ...
                if (FLAGS_unroll) {
                    ...
                    partition_unrolled(data, data_size, mask, shift, sum_hist, partitioned, &copy_cycle_counter);
                } else {
                    PerfEventBlock e{data_size, params, print_header, &copy_cycle_counter};
                    partition(data, data_size, mask, shift, sum_hist, partitioned, &copy_cycle_counter);
                }
                ...
                if (FLAGS_user_check) {
                    ...
                    if (FLAGS_unroll) {
                        ...
                        ret = ecall_partition_user_check_unrolled(eid, data, data_size, sum_hist, partitioned, mask, shift,
                                                                  &copy_cycle_counter_user_check);
                        if (ret != SGX_SUCCESS) {
                            l.err("ecall_partition_user_check_unrolled failed");
                        }
                    } else {
                        PerfEventBlock e{data_size, params, print_header, &copy_cycle_counter_user_check};
                        ret = ecall_partition_user_check(eid, data, data_size, sum_hist, partitioned, mask, shift,
                                               &copy_cycle_counter_user_check);
                        if (ret != SGX_SUCCESS) {
                            l.err("ecall_partition_user_check failed");
                        }
                    }
                }
                ...
                if (FLAGS_unroll) {
                    ...
                    PerfEventBlock e{data_size, params, print_header, &hist_cycle_counter_enclave};
                    ret = ecall_histogram(eid, data_size, mask, shift, &hist_cycle_counter_enclave);
                    if (ret != SGX_SUCCESS) {
                        l.err("ecall_histogram failed");
                    }
                }
                ...
                if (FLAGS_unroll) {
                    ...
                    ret = ecall_partition_unrolled(eid, data_size, mask, shift, &copy_cycle_counter_enclave);
                    if (ret != SGX_SUCCESS) {
                        l.err("ecall_partition_unrolled failed");
                    }
                } else {
                    PerfEventBlock e{data_size, params, print_header, &copy_cycle_counter_enclave};
                    ret = ecall_partition(eid, data_size, mask, shift, &copy_cycle_counter_enclave);
                    if (ret != SGX_SUCCESS) {
                        l.err("ecall_partition failed");
                    }
                }
